...
with open("tps_report.csv", "w", newline="") as csvfile:
  ...
  for item in employees:
    item.update({"finished_review": "Yes"})
    if item["first_name"] == "Bill" or item["job_title"] == "Consultant":
      item.update({"performance_review" : "poor"})
    else:
      item.update({"performance_review" : "excellent"})
